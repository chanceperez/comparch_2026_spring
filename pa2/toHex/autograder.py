...
def generate_test_suite():
    ...
    generate_test ( 0, False, 8 )
    generate_test ( 1, False, 128 )
    generate_test ( 2, False, 2048 )
    generate_test ( 3, False, 32768 )
    generate_test ( 4, True, 8 )
    generate_test ( 5, True, 128 )
    generate_test ( 6, True, 2048 )
    generate_test ( 7, True, 32768 )
        ...
        if test_toHex(1,path,verbose):
            ...
            if test_toHex(2,path,verbose):
                ...
                if test_toHex(3,path,verbose):
                    score += 1
                    if test_toHex(4,path,verbose):
                        score += 1
                        if test_toHex(5,path,verbose):
                            score += 1
                            if test_toHex(6,path,verbose):
                                score += 1
                                if test_toHex(7,path,verbose):
                                    score += 1

                                    for filenum in range(8,25):
                                        generate_test ( filenum, bool(random.getrandbits(1)), path=path )
                                        if test_toHex(filenum,path,verbose):
                                            score += 1
